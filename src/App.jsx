...
function App() {
  ...
  const initializePeer = (idToUse = null) => {
    ...
    peer.on('error', (err) => {
      ...
      if (err.type === 'peer-unavailable') {
        setErrorObj({ message: `Peer "${peerId}" not found. Is it online?` })
        setStatus('Connection failed')
      } else if (err.type === 'unavailable-id') {
        setErrorObj({ message: `ID "${idToUse}" is taken. Try another.` })
        setStatus('Registration failed')
      } else {
        setErrorObj({ message: `Error: ${err.type}` })
      }
    })
  ...
  const setupConnectionListeners = (connection) => {
    ...
    connection.on('close', () => {
      ...
      setConn(null)
      setScreen('connect') // Go back to dashboard
